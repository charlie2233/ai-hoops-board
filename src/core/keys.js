export const LONG_PRESS_MS = 600;

export const THEME_KEY = 'uiThemeV1';
export const THEME_STYLE_KEY = 'uiThemeStyleV1';
export const LANG_KEY = 'uiLangV1';
export const PLAYER_SIZE_KEY = 'uiPlayerSizeV1';
export const IMMERSIVE_SIDE_KEY = 'uiImmersiveSideV1';
export const LAYOUT_MODE_KEY = 'uiLayoutModeV1';
export const PLAY_FAVORITES_KEY = 'playFavoritesV1';
export const PLAY_RECENTS_KEY = 'playRecentsV1';
export const PLAY_RECENTS_LIMIT = 12;

export const SAVE_KEY = 'boardState_v1';
export const SAVE_SCHEMA = 1;
export const SAVE_TTL = 14 * 24 * 3600 * 1000;

export const EXPORT_OPTS_KEY = 'exportOpts_v1';
export const PATHING_AVOIDANCE_KEY = 'pathingAvoidance';

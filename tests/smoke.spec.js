...
test('keyboard shortcuts drive core board actions', async ({ page, browserName }) => {
  ...
  const defendersRemovedBefore = await page.evaluate(() => window.__aiHoopsBoardApp.areDefendersRemoved());
  await page.keyboard.press('d');
  await expect
    .poll(async () => page.evaluate(() => window.__aiHoopsBoardApp.areDefendersRemoved()))
    .toBe(!defendersRemovedBefore);
  ...
  await expect
    .poll(async () => page.evaluate(() => document.body.classList.contains('immersive')))
    .toBe(!immersiveBefore);
